...
Time::Time() : x(0), y(0), z(0), r(0) {}
Time::Time(double x, double y, double z, double r) {} 
...
ostream& Time::otput(ostream& out) const {	
	out << ":";
	out << "Hour: " << GetX();
	out << "Minute: " << GetY();
	out << "Seconds: " << GetZ();
...
double Time::SecondsTransfer() {
	double C = ((GetX() * 60 * 60) + (GetY() * 60) + GetZ());
	return C;
}

double Time::SecondsMinute() {
...
int Time::Comparison(const Time A, const Time B) const {
	if (A > B)
		return 1;
	if (A < B)
		return 2;
