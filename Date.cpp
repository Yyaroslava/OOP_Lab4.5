...
Date::Date() : x(0), y(0), z(0), r(0) {} 
Date::Date(double x, double y, double z, double r) { }
...
ostream& Date::otput(ostream& out) const {
	out << ":";
	out << " Years: " << GetX();
	out << " Month: " << GetY();
	out << " Day: " << GetZ();
...
int Date::Comparison(const Date A, const Date B) const {
	if (A > B)
		return 1;
	if (A < B)
		return 2;
